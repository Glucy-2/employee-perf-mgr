...
def encrypt(content):
    return sha256(
        content.encode(codec)
    ).hexdigest()  # 先将字符串编码为字节流（encode），再进行加密，然后将加密后的字节流进行摘要（hexdigest）


# 初始化，检查账户文件是否存在，不存在则写入默认账户，返回值为发生的错误信息和是否尝试创建了默认账户
def init():
    if not os.path.isfile(password_file) or not os.path.getsize(
        password_file
    ):  # 账户文件对应的路径不是文件（是文件夹或者不存在）或文件为空
